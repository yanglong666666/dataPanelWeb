...
projzh.ll2bll= function(input, opt_output, opt_dimension) {
    return bd09.fromWGS84(input, opt_output, opt_dimension);
};

projzh.bll2ll = function(input, opt_output, opt_dimension) {
    return bd09.toWGS84(input, opt_output, opt_dimension);
};

projzh.smerc2bll= function(input, opt_output, opt_dimension) {
    var output = sphericalMercator.inverse(input, opt_output, opt_dimension);
    return bd09.fromWGS84(output, output, opt_dimension);
};

projzh.bll2smerc = function(input, opt_output, opt_dimension) {
    var output = bd09.toWGS84(input, opt_output, opt_dimension);
    return sphericalMercator.forward(output, output, opt_dimension);
};
...
var baiduLngLatProj = new ol.proj.Projection({
    code: 'bd09ll', //百度经纬度坐标
    extent: ol.extent.applyTransform(extent, bd09.fromWGS84),
    units:'degree'
});

ol.proj.addProjection(baiduLngLatProj);
ol.proj.addCoordinateTransforms('EPSG:4326', baiduLngLatProj, projzh.ll2bll, projzh.bll2ll);
ol.proj.addCoordinateTransforms('EPSG:3857', baiduLngLatProj, projzh.smerc2bll, projzh.bll2smerc);
...
var baiduLayer = new ol.layer.Tile({
    source: new ol.source.XYZ({
        ...
        tileUrlFunction: function(tileCoord) {
            ...
            var style = "t:road|e:all|v:off";

            if(z>10)
            {
                style ="t:road|e:all|v:on";
            }
            ...
            return "http://api"+index+".map.bdimg.com/customimage/tile?x=" + x
                + "&y=" + y + "&z=" + z
                + "&udt="+dt+"&scale=1&ak=SolhsW6OnojQ7B4Wo7q1XLpp3emV3mXo"
                + "&styles="+style+"&t="+dt
        },
...
//百度影像图
var bdMapSatLayer = new ol.layer.Tile({
    title: "百度影像地图",
    source: new ol.source.BaiduMap({"mapType":"sat"})
});

//百度线划图
var bdMapVecLayer = new ol.layer.Tile({
    title: "百度线划地图",
